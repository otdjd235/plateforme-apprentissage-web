...
INSERT INTO `videos_complete` VALUES (1,22,'2025-04-11 07:36:24');
...
--
/*!50003 DROP PROCEDURE IF EXISTS `update_progression_user` */;
/*!50003 SET @saved_cs_client      = @@character_set_client */ ;
/*!50003 SET @saved_cs_results     = @@character_set_results */ ;
/*!50003 SET @saved_col_connection = @@collation_connection */ ;
/*!50003 SET character_set_client  = utf8mb4 */ ;
...
/*!50003 SET sql_mode              = 'ONLY_FULL_GROUP_BY,STRICT_TRANS_TABLES,NO_ZERO_IN_DATE,NO_ZERO_DATE,ERROR_FOR_DIVISION_BY_ZERO,NO_ENGINE_SUBSTITUTION' */ ;
DELIMITER ;;
CREATE DEFINER=`root`@`localhost` PROCEDURE `update_progression_user`(IN user_id INT, IN course_id INT)
BEGIN
    DECLARE total_elements INT;
    DECLARE completed_elements INT;
    DECLARE new_progression DECIMAL(5,2);

    -- Compter le nombre total de vidéos et d'exercices du cours
    SELECT COUNT(*) * 2 INTO total_elements
    FROM chapitres
    WHERE id_cours = course_id;

    -- Compter le nombre total de vidéos et d'exercices complétés par l'utilisateur
    SELECT 
        (SELECT COUNT(*) FROM videos_complete WHERE id_user = user_id 
         AND id_vid IN (SELECT id_vid FROM videosCours WHERE id_chap IN (SELECT id_chap FROM chapitres WHERE id_cours = course_id)))
        +
        (SELECT COUNT(*) FROM exercices_complete WHERE id_user = user_id 
         AND id_exo IN (SELECT id_exo FROM exercice WHERE id_chap IN (SELECT id_chap FROM chapitres WHERE id_cours = course_id)))
    INTO completed_elements;

    -- Calculer la progression
    IF total_elements > 0 THEN
        SET new_progression = (completed_elements / total_elements) * 100;
    ELSE
        SET new_progression = 0;
    END IF;

    -- Mettre à jour la table progression
    UPDATE progression
    SET progression = new_progression
    WHERE id_user = user_id AND id_cours = course_id;
END ;;
DELIMITER ;
/*!50003 SET sql_mode              = @saved_sql_mode */ ;
/*!50003 SET character_set_client  = @saved_cs_client */ ;
...
-- Dump completed on 2025-04-11  4:23:17
